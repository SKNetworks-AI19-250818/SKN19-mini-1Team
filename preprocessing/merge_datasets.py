...
def build_final_dataset(mode="train"):
    ...
    if not activity_history.empty:
        activity_balance = activity_history.groupby('TRAVEL_ID')['ACTIVITY_TYPE_CD'].apply(calculate_entropy).rename('activity_balance_index')
        final_df = final_df.merge(activity_balance, on='TRAVEL_ID', how='left')
        final_df['activity_balance_index'] = final_df['activity_balance_index'].fillna(0)
    else:
        final_df['activity_balance_index'] = 0
    ...
    final_columns = [
        ...
        "activity_balance_index", # New feature
    ...
    # --- Calculate Travel Intensity Index ---
    intensity_features = ['activity_history_rows_per_day', 'move_cnt_per_day']
    if all(c in final_df.columns for c in intensity_features) and not final_df[intensity_features].isnull().values.any():
        scaler = StandardScaler()
        # Create a temporary df to avoid SettingWithCopyWarning
        temp_scaled = scaler.fit_transform(final_df[intensity_features])
        final_df['travel_intensity_index'] = temp_scaled.sum(axis=1)
    else:
        final_df['travel_intensity_index'] = 0

    all_final_columns = existing_final_columns + per_day_columns + ['travel_intensity_index']
